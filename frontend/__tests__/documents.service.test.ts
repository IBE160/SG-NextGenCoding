...
import { getSummary, getSummaryStatus, uploadDocument } from '../src/services/documents';
...
describe('documents service', () => {
  ...
  describe('uploadDocument', () => {
    it('should upload a file successfully with user ID and access token', async () => {
      ...
      const mockResponse = { document_id: mockDocumentId, message: 'Upload initiated' };
    ...
    it('should throw an error if upload fails', async () => {
      ...
      await expect(uploadDocument(mockFile)).rejects.toThrow(`Upload failed: ${errorMessage}`);
    });
  });

  describe('getSummary', () => {
    it('should fetch summary successfully', async () => {
      const mockSummary = { document_id: mockDocumentId, summary_text: 'Summary content' };
    ...
    it('should throw an error if fetching summary fails', async () => {
      ...
      await expect(getSummary(mockDocumentId)).rejects.toThrow(`Failed to get summary: ${errorMessage}`);
    });
  });
  ...
  describe('getSummaryStatus', () => {
    ...
    it('should throw an error if fetching summary status fails', async () => {
      ...
      await expect(getSummaryStatus(mockDocumentId)).rejects.toThrow(`Failed to get summary status: ${errorMessage}`);
    });
  });
});