...
describe('Document Service', () => {
  ...
  describe('getSummary', () => {
    it('should fetch the summary successfully', async () => {
      ...
      expect(mockedAxios.get).toHaveBeenCalledWith('/api/v1/documents/123/summary', {
    ...
    it('should handle errors when fetching the summary', async () => {
      ...
      await expect(getSummary('123', 'test-token')).rejects.toThrow('An unexpected error occurred while getting summary: Failed to fetch');
    });
  });
  ...
  describe('getSummaryStatus', () => {
    it('should fetch the summary status successfully', async () => {
      ...
      expect(mockedAxios.get).toHaveBeenCalledWith('/api/v1/documents/123/status', {
        headers: { Authorization: 'Bearer test-token' },
      });
    ...
    it('should handle errors when fetching the summary status', async () => {
      ...
      await expect(getSummaryStatus('123', 'test-token')).rejects.toThrow('An unexpected error occurred while getting summary status: Failed to fetch');
    });
  });
});
