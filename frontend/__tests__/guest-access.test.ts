...
describe('Guest Access for Core Features', () => {
  ...
  it('should allow guest access for the first use and increment usage', async () => {
    ...
    ;(incrementUsageCount as jest.Mock).mockImplementation(() => {
      ;(getUsageCount as jest.Mock).mockReturnValue(
        (getUsageCount() as number) + 1,
      ) // Simulate actual increment
    })
    ...
    expect(
      screen.getByText(/You have 0 out of 2 free uses./i),
    ).toBeInTheDocument() // Initial state is 0

    // Simulate clicking the feature button
    await userEvent.click(
      screen.getByRole('button', { name: /Use Core Feature/i }),
    )

    expect(incrementUsageCount).toHaveBeenCalledTimes(1)
    await waitFor(() => {
      expect(
        screen.getByText(/You have 1 out of 2 free uses./i),
      ).toBeInTheDocument() // After click, it's 1
    })
  ...
  it('should allow guest access up to the limit and increment usage', async () => {
    ...
    ;(checkUsageLimit as jest.Mock).mockImplementation(
      () => currentUsage >= GUEST_LIMIT,
    )
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Guest Mode - Access Blocked/i),
      ).toBeInTheDocument()
      expect(
        screen.getByText(/You have used your 2 free accesses./i),
      ).toBeInTheDocument()
  ...
  it('should not affect authenticated users by guest limit', async () => {
    ...
    expect(
      screen.queryByText(/Guest Mode - Limited Access/i),
