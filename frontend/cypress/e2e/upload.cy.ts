...
describe('File Upload Interface E2E Tests', () => {
  ...
  it('allows a logged-in user to upload a valid file and sees "processing" status', () => {
    ...
    cy.window().then((win) => {
      win.localStorage.setItem('supabase.auth.token', '{"some_token": "mock_token"}');
    });
    ...
    cy.get('p').should('contain', 'File uploaded successfully! Document ID: a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11');
  });
  ...
  it('allows a guest user to upload a valid file within the limit', () => {
    ...
    cy.get('h1').should('contain', 'Upload Your Lecture Notes');
    cy.get('[aria-label="file upload area"]').should('be.visible');
    cy.get('p').should('contain', 'As a guest, you have 2 free uploads remaining.');
    ...
    cy.get('p').should('contain', 'As a guest, you have 1 free uploads remaining.'); // Limit reduced
  });
  ...
  it('prompts a guest user to register/login after exceeding the upload limit', () => {
    ...
    cy.wait('@uploadFile1');
    cy.get('p').should('contain', 'File uploaded successfully!');
    cy.get('p').should('contain', 'As a guest, you have 1 free uploads remaining.');
    ...
    cy.get('div[role="alert"]').should('contain', 'Please log in or register to continue uploading files.');
  ...
  it('simulates uploading an unsupported file type and verifies error message', () => {
    cy.intercept('POST', '/api/v1/documents/upload', {
      statusCode: 400,
      body: {
        detail: 'Unsupported file type: image/jpeg. Allowed types are: application/pdf, text/plain, application/vnd.openxmlformats-officedocument.wordprocessingml.document',
      },
    }).as('uploadFile');
    ...
    cy.get('p').should('contain', 'Upload failed: Unsupported file type: image/jpeg. Allowed types are: application/pdf, text/plain, application/vnd.openxmlformats-officedocument.wordprocessingml.document');
  });
  ...
  it('simulates uploading an oversized file and verifies error message', () => {
    ...
    cy.get('input[type="file"]').then((input) => {
      const blob = Cypress.Blob.arrayBufferToBlob(new ArrayBuffer(twentyOneMB), 'application/pdf');
    ...
    cy.get('p').should('contain', 'Upload failed: File size exceeds limit. Max size is 20MB.');
  });
});